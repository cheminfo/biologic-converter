import {IOBufer} from "iobuffer";

import { TechniqueLookUp } from "./techniqueHelpers/techniqueLookUp";
import { readType as pValue } from "./readType";

// can be improved by having an interface that for each technique as a specific type
interface ParseTechnique { technique:string, [key:string]:string }
/** 
...
export function parseTechnique(
  ...
  for (const off of [0x572, 0x1845, 0x1846]) {//byte flags params' start
